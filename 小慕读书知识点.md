...
* 第三步：父组件调用onSuccess方法拿到返回值后，重新渲染页面表单数据显示在页面上

#### 后端接口：电子书上传

* 第一步：在后端添加新的路由/book, 完善/router/book.js，在 book.js内添加新的post接口`/upload`，接口使用了multer（multer封装了文件上传操作，操作简便，只需要提供nginx文件目录，如`multer({dest:'/Usr/liqian/admin-imooc-book/book/'}).single('file')`,一般情况下后端无法直接访问系统文件目录，因此这个接口使用了nginx反向代理，需要在nginx配置好文件目录，使得multer可以直接访问接口文件目录）,接口再返回response
