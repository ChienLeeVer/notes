...
* 第一步：**路由跳转渲染页面数据**： 点击编辑电子书，跳转到相关路由，在路由接口中需要携带参数，这个参数可以用作数据库查询标识，如fileName，并且在进入编辑页面后在created生命周期根据这个fileName调用methods中的getBook方法,而getBook方法则调用封装好的后端接口api方法（@/api/book.js）（这里直接调用api/book.js，因为不需要临时存储数据，因此无需通过vuex），api接口也是调用封装好的axios方法，如此前端在getBook.then的回调方法获得返回值后再调用在methods中定义好的setData方法给响应式数据赋值，vue会重新渲染页面
