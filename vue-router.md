...
\\配置路由，说明：配置项为数组，数组元素为对象，每个对象包含两个属性path地址和component需要显示的组件
const routes = [
    { path: '/', component: home },
...
import Vuerouter from 'vue-router' //重要：引入路由
import Vue from 'vue'
import App from './App.vue'

Vue.use(Vuerouter) //重要：注册路由
...
//注意：目的地和当前路由相同，只有参数发生了改变，那么组件将会被复用而不是销毁重建，因此生命周期函数将不会被再次调用，因此监听属性变化就用watch:{'$route': '函数名'}/beforeRouteUpdate(to, from, next){}
```
...
### 嵌套路由
1.修改路由配置组件route.vue。
目的：/user/:id/profile 或者 /user/:id/home.
实现思路：为/user/:id路由添加子路由，先分别创建一个userProfile.vue和userHome.vue，然后在router.vue中分别引入。接着在routes数组中的user路由添加属性```chilrend: [ { path:'', component: userHome }, { path: 'profile', component: userProfile } ]```。
...
2.然后在user.vue中使用router-view,当匹配到/user/:id/profile时将会显示对应的内容
...
### 编程式导航
除了router-link创建导航实现跳转外，还可以有以下方法：
...
this.$router.push({
    path：`/user/${user.id}`   //路由：user/123 ，假设user.id 为123
})
this.$router.push({
    name: 'user',
    params: {id: 123}  //路由：user/123 ，注意params只能和name一起使用
})
this.$router.push({
    path:'user',
    query:{ id:123 } //查询路由：/user?id=123
})
```
...
### 命名视图
是什么:能够为```<router-view name=""></router-view>```定义一个名字,如果没命名，默认名default

为啥用:使得在同一个组件当中使用多个同级视图。例如现在App.vue中只有一个router-view,这意味着当输入/about时只能显示一个视图即只能显示about.vue一个组件。现在我想输入/about时让多个组件都显示。
