...
### 输入url地址到显示页面的过程
1. url地址解析验证：解析协议、域名、端口、文件地址、参数、hash值，验证是否合法
...
1. 三次握手：
    ...
    1.  第一次握手：客户端向服务端发送SYN报文，并指明了客户端的初始化序列号，此时客户端处于SYN_WAIT1状态

    2.  第二次握手：服务端接收到了客户端发送的SYN报文后，能够确认客户端具有发送能力、服务端具有接受能力，为了让客户端知道服务端的收发能力，发送一个服务端的SYN报文，并且在报文中指明了服务端的初始化序列号，ack值为客户端的初始化序列号+1

    3. 第三次握手：客户端接收到了服务端的SYN报文，确认了客户端服务端的收发能力，但此时服务端仍不知道客户端是否有接收能力。客户端向服务端发送ACK报文，报文中ack值为服务端初始化序列号+1。seq值为客户端初始化序列号+1。此时双方连接已建立。

2. 四次挥手：

    1.  第一次挥手： 客户端向服务端发送FIN报文，报文中指明了一个序列号。此时客户端停止发送数据，等待确认。

    2.  第二次挥手： 服务端接收到了客户端发送的FIN报文后，会向客户端发送一个ACK报文，值为客户端序列号+1。意在告诉客户端已经接收到关闭连接通知，等待已经发送的数据传输完毕，一会儿再告知客户端。

    3.  第三次挥手：服务端向客户端发送一个FIN报文，并指明了服务端的序号，意在告诉客户端，数据传输完毕，可以关闭连接了。

    4.  第四次挥手：客户端接收到服务端的FIN报文后，同样发送一个ACK报文，值为服务端的序列号+1。服务端接收到报文后关闭连接，过段时间客户端没接收到服务端的发送的报文也关闭连接。